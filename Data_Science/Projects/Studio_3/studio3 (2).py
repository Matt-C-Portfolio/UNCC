...
def boxplot(df, column_name, 
    ...
    """
    plt.figure(figsize=(5, 6), facecolor=background_color)  # Set the figure background color
    ...
    plt.boxplot(df[column_name], vert=False,
                ...
                flierprops=dict(marker='o', markerfacecolor=flier_color, markersize=1))

    plt.title(f'Boxplot of {column_name}', color=text_color, fontsize=12)
    plt.xlabel(column_name, color=text_color)
...
def plot_quantitative_data(df, columns, 
    ...
    fig.suptitle('Quantitative Data Visualizations', color=suptitle_color, fontsize=48)
    ...
    for i, col in enumerate(columns):
        ...
        df.boxplot(column=col, patch_artist=True,
                   boxprops=dict(facecolor='white', color=box_color),
                   medianprops=dict(color=median_color, linewidth=1.5),
                   whiskerprops=dict(color=whisker_color, linewidth=2),
                   capprops=dict(color=whisker_color, linewidth=2),
        ...
        ax[row_index * 2, col_index].set_title(col, color=box_color, fontsize=30)
        ...
        ax[row_index * 2, col_index].tick_params(axis='y', colors=box_color, labelsize=24)
        ...
        #ax[row_index * 2 + 1, col_index].set_title(col, color=box_color, fontsize=36)
        ...
        ax[row_index * 2 + 1, col_index].tick_params(colors=box_color, labelsize=24)
        ax[row_index * 2 + 1, col_index].grid(visible=False)
        ax[row_index * 2 + 1, col_index].set_xlabel('', visible=False)
        ax[row_index * 2 + 1, col_index].set_ylabel('Count', fontsize=24)
...
def scatterVsTarget(data, target_variable, nrows=3, ncols=3, title='Scatterplots of Predictors vs Target',
                           face_color='white', title_color='black', line_color='red', scatter_color='blue'):
    """
    Plot scatter plots of each feature against the target variable.

    Parameters:
    - data: DataFrame containing the features
    - target_variable: Series of the target variable
    - nrows: Number of rows for subplots
    - ncols: Number of columns for subplots
    - title: Title of the entire plot
    - face_color: Background color of the plots
    ...
    fig.suptitle(title, color=title_color, fontsize=22)

    # Extract feature names excluding the target variable
    attributes = data.columns[data.columns != target_variable.name]

    # Flatten the axes array for easy iteration
    axes = axes.flatten()

    # Loop through each feature and create scatter plots
    for ax, feature in zip(axes, attributes):
        ax.set_facecolor(face_color)
        ax.scatter(data[feature], target_variable, color=scatter_color, alpha=0.3)
        ax.set_xlabel(feature, color=title_color, fontsize=16)
        ax.set_ylabel(target_variable.name, color=title_color, fontsize=16)
        ...
        ax.tick_params(axis='both', colors=title_color, labelsize=16)

        # Fit line for visualization
        if len(data[feature]) > 1:
            # Calculate regression line
            z = np.polyfit(data[feature], target_variable, 1)
            p = np.poly1d(z)
            ax.plot(data[feature], p(data[feature]), color=line_color)
...
def plotResiduals(models, X_test1, X_test2, y_test, model_titles=None, 
                   title="Normality of Residuals", 
                   face_color='white', title_color='black', 
                   line_color='red', scatter_color='blue', text_color='black'):
    """
    Plot Q-Q, distribution, and KDE plots for residuals of a list of models, using a separate test set for the first model.

    Parameters:
    - models: List of trained model objects
    - X_test1: Test features for the first model
    - X_test2: Test features for other models
    - y_test: Actual target values for test set
    - model_titles: List of titles for each model (optional)
    - title: Overall title for the plot
    - face_color: Background color for plots
    - title_color: Color for main title
    - line_color: Color for regression line in Q-Q plot and KDE line
    - scatter_color: Color for scatter plot points and histogram
    - text_color: Color for text and axis labels
    """
    # Set default titles if none provided
    if model_titles is None:
        model_titles = [f"Model {i+1}" for i in range(len(models))]

    # Set up the figure with rows based on number of models and 3 columns for each type of plot
    n_models = len(models)
    fig, axs = plt.subplots(nrows=n_models, ncols=3, figsize=(15, 5 * n_models))
    fig.patch.set_facecolor(face_color)
    fig.suptitle(title, color=title_color, fontsize=36)
    
    # Ensure axs is a 2D array even if there's only one row
    if n_models == 1:
        axs = [axs]

    # Loop through each model, using X_test1 for the first model, and X_test2 for the rest
    for i, (model, model_title) in enumerate(zip(models, model_titles)):
        X_test = X_test1 if model is models[0] else X_test2
        ...
        # Q-Q Plot
        stats.probplot(residuals, dist="norm", plot=axs[i][0])
        axs[i][0].set_title(f'Q-Q Plot - {model_title}', color=text_color, fontsize=16)
        axs[i][0].set_xlabel('Expected Quantiles', color=text_color, fontsize=16)
        axs[i][0].set_ylabel('Observed Quantiles', color=text_color, fontsize=16)
        axs[i][0].get_lines()[1].set_color(line_color)
        axs[i][0].get_lines()[1].set_linewidth(3)
        axs[i][0].get_lines()[0].set_color(scatter_color)

        # Distribution of residuals
        sns.histplot(residuals, kde=True, ax=axs[i][1], color=scatter_color)
        axs[i][1].set_title(f'Distribution of Residuals - {model_title}', color=text_color, fontsize=16)
        axs[i][1].set_xlabel('Residual', color=text_color, fontsize=16)
        axs[i][1].set_ylabel('Frequency', color=text_color, fontsize=16)
        axs[i][1].get_lines()[0].set_color(line_color)
        axs[i][1].get_lines()[0].set_linewidth(3)

        # KDE Plot
        sns.kdeplot(residuals, ax=axs[i][2], color=line_color)
        axs[i][2].set_title(f'KDE Plot - {model_title}', color=text_color, fontsize=16)
        axs[i][2].set_xlabel('Residual', color=text_color, fontsize=16)
        axs[i][2].set_ylabel('Density', color=text_color, fontsize=16)
        axs[i][2].get_lines()[0].set_linewidth(3)

        # Adjust plot aesthetics for each subplot
        for j in range(3):
            axs[i][j].spines['bottom'].set_color(text_color)
            axs[i][j].spines['top'].set_color(face_color)
            axs[i][j].spines['right'].set_color(face_color)
            axs[i][j].spines['left'].set_color(text_color)
            axs[i][j].set_facecolor(face_color)
            axs[i][j].tick_params(axis='y', colors=text_color, labelsize=16)
            axs[i][j].tick_params(axis='x', colors=text_color, labelsize=16)
